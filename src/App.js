...
class App extends Component {
  ...
  visitCountries = countryDetails => {
    this.setState(prevState => ({
      visitList: [...prevState.visitList, countryDetails],
    }))
  }

  removeCountry = name => {
    const {visitList} = this.state
    const filteredData = visitList.map(each => each.name !== name)

    this.setState({visitList: filteredData})
  }
    ...
    return (
      <div className="main-bg-container">
        ...
        <ul className="visited-list-container">
          {visitList.map(eachVisitCountry => (
            <VisitedCountries
              visitedCountry={eachVisitCountry}
              key={eachVisitCountry.name}
